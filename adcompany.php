...
if(isset($_POST['submit'])){
  ...
  if(empty($cname) == true){
    $msgerror .= '<br>Company name can not be empty';
    $error++;
  }

  if(empty($locat) == true){
    $msgerror .= '<br>Location can not be empty';
    $error++;
  }

  if(empty($email) == true){
    $msgerror .= '<br>Email can not be empty';
    $error++;
  }
  else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
    $msgerror .= '<br>Invalid Email';
    $error++;
  }

  if(empty($address) == true){
    $msgerror .= '<br>Address can not be empty';
    $error++;
  }

  if(empty($bio) == true){
    $msgerror .= '<br>Company Bio can not be empty';
    $error++;
  }
  else if(strlen($bio) > 160){
    $msgerror .= '<br>Company Bio can not be more than 160 characters';
    $error++;
  }

  if(empty($logo) == false){
    $fileupdt = $ax->checkfile($logo,$logo_size);
    if($fileupdt == 0){
        $msgerror .= "<br>Invalid file format (Allowed png,jpg,jpeg)";
        $error++;
    }
    else if($fileupdt == 2){
        $msgerror .= "<br>Company logo size too big (Max image size 2MB)";
        $error++;
    }
  }
  else{
    $msgerror .= '<br>Company logo can not be empty';
    $error++;
  }

  if($error == 0){
    $inad = $ax->add_company($apid, $bio, $cname, $locat, $email, $address, $logo, $logo_temp);
    if($inad == 1){
      $msg = '<script type="text/javascript">
              swal("Company Created!", "Company added successfully.", "success");
            </script>';
    }
    else{
      $msg = '<script type="text/javascript">
...
if(isset($_GET['q']) && empty($_GET['q']) == false){
  $search = $_GET['q'];
  $company_list = $ax->get_companies($search, $pagex);
}
else{
  $company_list = $ax->get_companies($search, $pagex);
}
  ...
  <body>
    ...
    <div class="br-mainpanel">
      <div class="br-pagetitle">
        ...
        <div>
          <h4>Companies</h4>
          <p class="mg-b-0">List of all the companies on Rook+</p>
          <button onclick="add()" class="btn btn-primary btn-with-icon mg-t-10">
            <div class="ht-40">
              ...
              <span class="pd-x-15">Add New Company</span>
            </div>
          </button>
        </div>
      </div>
        ...
        <div class="row row-sm mg-t-20">
          <div class="col-lg-8">
            <div class="card bd-0 shadow-base">
              <div class="pd-x-25 pd-t-25">
                <h6 class="tx-13 tx-uppercase tx-inverse tx-semibold tx-spacing-1 mg-b-20">Companies 
                ...
                <hr>
                <?php echo $company_list['company'].$company_list['range'].$company_list['page'];?>
          ...
          <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            ...
            <div class="card shadow-base card-body pd-25 bd-0">
              <div class="row">
                <div class="col-sm-12">
                  <h6 class="card-title tx-uppercase tx-12">Total Companies</h6>
                  <p class="display-4 tx-medium tx-inverse mg-b-5 tx-lato"><?php echo $companies['active'].'<span style="font-size: 25px;">/'.$companies['total'].'</span>';?></p>
                  <p class="tx-12">This shows <b>active/total</b> number of companies on Rook+</p>
                </div><!-- col-6 -->
              </div><!-- row -->
            </div>
