...
/**
 * @api {get} /lists/recommended/internships ListRecommendedInternships
 * @apiVersion 0.0.1
 * @apiName ListRecommendedInternships
 * @apiGroup Lists Recommendation
 * @apiPermission none
 *
 * @apiDescription Returns the recommended internships for a specific user based on profile on the platform and the short details to populate a list. Also contains if user has applied
 *
 ...
 * @apiSuccess {Number="1-4"} result.internships.priority Priority number indicates the level of best match. 4 being highest
...
if ($jwt) {
    try {
        ...
        $data = $student->getRecommendedInternships();
