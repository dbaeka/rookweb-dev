...
if(isset($_GET['pass']) && $_GET['pass']){
  $token = $_GET['pass'];
  $cname = $val->reset_password_info($token);
  if($cname == '0'){
    $msg = '<script type="text/javascript">
              swal("Error", "Link has expired or is invalid", "error");
            </script>';
    $cname = '';
  }
}
...
if(isset($_POST['activate'])){
  ...
  if($error == 0){
    $udata = $val->set_new_password($token, $password);
    if($udata == 1){
      $msg = '<script type="text/javascript">
                  swal({
    ...
    <div class="row no-gutters flex-row-reverse ht-100v">
      <div class="col-md-6 bg-gray-200 d-flex align-items-center justify-content-center">
        <div class="login-wrapper wd-250 wd-xl-350 mg-y-30">
          ...
          <p class="tx-center mg-b-60">Hi <b><?php echo $cname;?></b>, Set a new password for your account here.</p>
          <?php echo $msg;?>
